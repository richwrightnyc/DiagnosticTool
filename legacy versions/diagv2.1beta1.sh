...
# 	echo "Active Interface: $intName" >> ~/Desktop/$serial.txt
	...
	interfaceMAC=$(
		...
		| grep -A 1 "$activeInterface" \
		| grep "Ethernet Address:" \
		| awk '{print $3}'
		)

	echo "MAC Address: $interfaceMAC" >> ~/Desktop/$serial.txt
...
# 		| head -15 \
# 		| grep "Hardware (MAC) Address:" \
# 		| awk '{print $4}'
# 		)
# 
# 	echo "MAC Address: $nicMAC" >> ~/Desktop/$serial.txt
